...
public class TrackerBehaviour : MonoBehaviour
{
   ...
   [StructLayout(LayoutKind.Sequential, Pack = 1)]
    ...
    void OnDisable()
    {
        // Note we don't deinitialize either tracker or camera:
        stopTracker();
    ...
    private void UpdateImageContainer()
    {
        ...
        CameraDeviceBehaviour camera = (CameraDeviceBehaviour)FindObjectOfType(
        ...
        if (mNumImageHeaders != camera.GetAllImages().Count ||
           (camera.GetAllImages().Count > 0 && mImageHeaderData == IntPtr.Zero))
        {

            mNumImageHeaders = camera.GetAllImages().Count;
        ...
        int i = 0;
        foreach (Image image in camera.GetAllImages().Values)
        {
