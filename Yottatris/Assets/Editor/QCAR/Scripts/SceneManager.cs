...
// It uses a thread safe singleton pattern as described here:
// http://msdn.microsoft.com/en-us/library/ff650316.aspx (Static Initialization)
public sealed class SceneManager
{
    ...
    private static readonly SceneManager mInstance = new SceneManager();
    ...
    public ConfigData.ImageTarget GetImageTarget(string name)
    {
        ConfigData.ImageTarget it;
        if (!ConfigDataManager.Instance.GetImageTarget(name, out it))
        {
            // Return default Image Target if name cannot be found.
    ...
    public ConfigData.MultiTarget GetMultiTarget(string name)
    {
        ConfigData.MultiTarget mt;
        if (!ConfigDataManager.Instance.GetMultiTarget(name, out mt))
        {
            // Return default Multi Target if name cannot be found.
    ...
    public ConfigData.FrameMarker GetFrameMarker(int id)
    {
    ...
    private List<ConfigData.MultiTargetPart> CreateDefaultParts()
    {
        ...
        ConfigData.ImageTarget it = GetImageTarget("");
